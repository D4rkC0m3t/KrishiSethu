...
export const AuthProvider = ({ children }) => {
  ...
  // FIXED: loadUserProfile function that avoids recursion and 406 errors
    ...
    try {
      ...
      for (const tableName of tableNames) {
        try {
          ...
          // Use the Supabase client correctly (this fixes the 406 error)
          ...
          if (data) {
            profile = data;
            error = null;
            console.log(`✅ Profile loaded from ${tableName} table:`, profile);
      ...
      // If no profile found in any table, create a default profile
      if (!profile || error) {
        ...
        // Use the first available table from our attempts
        for (const tableName of tableNames) {
          try {
            ...
            if (!createError) {
              ...
              console.warn(`⚠️ Could not create profile in ${tableName}:`, createError.message);
            }
          } catch (createErr) {
            console.warn(`⚠️ Exception creating profile in ${tableName}:`, createErr.message);
          }
        }
  ...
  // FIXED: Supabase auth functions (prevent manual user table insertion)
  const signup = async (email, password, userData = {}) => {
    try {
      console.log('📝 Starting signup process for:', email);
      
      // Use Supabase Auth (NOT manual insertion) - this fixes the 400 error
